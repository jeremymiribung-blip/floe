...
- Switched the Groq cleanup model from `llama-3.1-8b-instant` to `qwen/qwen3-32b`. Updated the cleanup system prompt to use a strict transcript-cleanup prompt with a technical vocabulary (Floe, Groq, Tauri, TypeScript, Rust, GitHub, Pull Request, Branch, Draft, Clipboard, Hotkey, Single Instance Lock, Whisper Large V3 Turbo, Llama 3.1 8B Instant, Qwen 3 32B, OpenCode, MiniMax, Codex, Parakeet) while still forbidding stylistic rewriting, JSON/YAML/Markdown output, labels, and commentary. Raised the dynamic `max_tokens` cap from 512 to 1024 for long German inputs and added `Sure,`, `Okay,`, and `Here you go,` to the rejected output prefixes so the fallback path still triggers on chatty preambles. STT, audio encoding, request flow, retries, fallback, diagnostics, and product surface are unchanged.
