...
- Fixed Groq → Cerebras → clipboard → paste flow as the standard path. The `cleanup_transcript` command now always runs the Groq transcript through Cerebras and copies the result to the clipboard; if Cerebras cleanup fails, the raw Groq transcript is pasted instead and a short `Cleanup failed` warning is shown.
...
- The `CleanupMode` enum and its `Raw`, `Fast`, and `Clean` variants.
...
- The `SettingsErrorCode::MissingCerebrasApiKey` variant and the `"missingCerebrasApiKey"` `SettingsError` code, since the new cleanup path never hard-errors on a missing key.
- Tests that asserted mode persistence, `mode: "raw" | "fast" | "clean"` return values, or force-fast fallback on Cerebras key removal.
