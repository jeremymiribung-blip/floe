...
Floe is Groq-only and intentionally minimal. There are no cleanup modes, no Behavior section, no Cerebras, no Qwen cleanup model, no GPT-OSS cleanup model, and no provider switching — the only flow is `Groq STT → Groq cleanup → clipboard → paste`.
